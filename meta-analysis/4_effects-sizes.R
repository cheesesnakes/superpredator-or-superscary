...
data_baci <- data %>%
    ...
    mutate(
        treatment = ifelse(treatment == "before treatment", "control", treatment),
...
data_tc <- data_tc %>%
    ...
    pivot_wider(names_from = treatment, values_from = c(mean, se, n)) %>%
    ungroup() %>%
    # apply multiplier to mean,
    mutate(
        multiplier = ifelse(is.na(multiplier), 1, -multiplier),
        mean_treatment = mean_treatment * multiplier,
        mean_control = mean_control * multiplier,
    )
...
data_tc_smd <- data_tc_smd %>%
    mutate(
        smd = ifelse(pop_cn == "Mouse" & is.na(smd), 0, smd),
