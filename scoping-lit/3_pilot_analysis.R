...
pilot <- pilot %>%
  mutate(
    exposure = ifelse(is.na(exposure), intervention, exposure),
...
pilot <- pilot %>%
  mutate(
    control = ifelse(grepl("none", control), "none", control),
...
pilot <- pilot %>%
  mutate(
    outcome = ifelse(grepl("activity times", outcome), "activity time", outcome),
